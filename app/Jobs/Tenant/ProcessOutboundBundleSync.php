...
class ProcessOutboundBundleSync implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            Log::info('Processing outbound bundle sync', [
                'tenant_id' => $syncQueue->tenant_id,
                'sync_queue_id' => $syncQueue->id,
                'bundle_id' => $syncQueue->syncable_id,
            ...
            Log::info('Outbound bundle sync completed', [
                'tenant_id' => $syncQueue->tenant_id,
                'sync_queue_id' => $syncQueue->id,
                'central_sync_id' => $response['sync_id'] ?? null,
            ]);
        } catch (\Exception $e) {
            Log::error('Outbound bundle sync failed', [
                'tenant_id' => $syncQueue->tenant_id,
                'sync_queue_id' => $syncQueue->id,
                'error' => $e->getMessage(),
            ...
            if ($syncQueue->canRetry()) {
                ...
                app(SyncFailureNotificationService::class)->notifyOwners($syncQueue, 'Bundle outbound sync');
            }
    ...
    {
        Log::error('ProcessOutboundBundleSync job failed permanently', [
            'sync_queue_id' => $this->syncQueueId,
