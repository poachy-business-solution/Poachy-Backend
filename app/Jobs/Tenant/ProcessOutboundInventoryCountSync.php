...
class ProcessOutboundInventoryCountSync implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($syncQueue->status === 'completed') {
            Log::info('InventoryCount sync already completed, skipping', [
        ...
        if ($syncQueue->isStale()) {
            ...
            Log::warning('InventoryCount sync is stale, marking as expired', [
                'sync_queue_id' => $syncQueue->id,
        ...
        try {
            ...
            Log::info('Processing outbound inventory count sync', [
            ...
            Log::info('Outbound inventory count sync completed', [
                'tenant_id' => $syncQueue->tenant_id,
                'sync_queue_id' => $syncQueue->id,
                'central_sync_id' => $response['sync_id'] ?? null,
            ]);
        } catch (\Exception $e) {
            Log::error('Outbound inventory count sync failed', [
                'tenant_id' => $syncQueue->tenant_id,
                'sync_queue_id' => $syncQueue->id,
                'error' => $e->getMessage(),
    ...
    {
        Log::error('ProcessOutboundInventoryCountSync job failed permanently', [
            'sync_queue_id' => $this->syncQueueId,
