...
class ProcessOutboundVariantSync implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (!$syncQueue) {
            Log::error('SyncQueueOutbound record not found for variant sync', [
        ...
        if (!$syncQueue->acquireLock($workerId)) {
            Log::info('Could not acquire lock for variant sync, another worker processing', [
                'sync_queue_id' => $syncQueue->id,
            ]);
            return;
        }
        ...
        try {
            ...
            Log::info('Processing outbound variant sync', [
                'tenant_id' => $syncQueue->tenant_id,
                'sync_queue_id' => $syncQueue->id,
                'variant_id' => $syncQueue->syncable_id,
            ...
            Log::info('Outbound variant sync completed', [
                'tenant_id' => $syncQueue->tenant_id,
                'sync_queue_id' => $syncQueue->id,
                'central_sync_id' => $response['sync_id'] ?? null,
            ]);
        } catch (\Exception $e) {
            Log::error('Outbound variant sync failed', [
                'tenant_id' => $syncQueue->tenant_id,
                'sync_queue_id' => $syncQueue->id,
                'error' => $e->getMessage(),
            ...
            if ($syncQueue->canRetry()) {
                ...
                Log::error('Max retries reached for variant sync, failed permanently', [
                    'sync_queue_id' => $syncQueue->id,
                    'retry_count' => $syncQueue->retry_count,
                ]);
            }
    ...
    /**
     * Send variant sync data to central API
     */
    protected function sendToCentralAPI(SyncQueueOutbound $syncQueue): array
    {
        $centralApiUrl = config('services.central_api.url') . '/api/v1/central/sync/inbound/variant';
        $apiToken = config('services.central_api.token');

        Log::debug('Sending variant sync to central API', [
            'url' => $centralApiUrl,
            'sync_queue_id' => $syncQueue->id,
        ]);
    ...
    {
        Log::error('ProcessOutboundVariantSync job failed permanently', [
            'sync_queue_id' => $this->syncQueueId,
