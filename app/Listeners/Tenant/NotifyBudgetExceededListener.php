...
class NotifyBudgetExceededListener
{
    ...
    public function handle(BudgetExceeded $event): void
    {
        try {
            ...
            if ($usersToNotify->isEmpty()) {
                Log::warning('No users to notify for budget exceeded', [
                    'budget_id' => $budget->id,
            ...
            Log::info('Budget exceeded notifications dispatched', [
                'budget_id' => $budget->id,
                'users_notified' => $usersToNotify->count(),
                'channel' => $channel,
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to send budget exceeded notifications', [
    ...
    private function prepareMessage(Budget $budget): array
    {
        ...
        $overage = number_format($budget->spent_amount - $budget->budget_amount, 2);

        $subject = "Budget Exceeded: {$budgetName}";

        $body = "A budget has been exceeded:\n\n";
        $body .= "Budget: {$budgetName}\n";
        $body .= "Store: {$storeName}\n";
        $body .= "Spent: KES {$spent} of KES {$total}\n";
        $body .= "Overage: KES {$overage}\n";
