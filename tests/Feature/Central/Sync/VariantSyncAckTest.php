...
class VariantSyncAckTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tenantId = 'variant-sync-test-'.uniqid();
        ...
        DB::connection('central')->table('domains')->insert([
            'domain' => 'variant-sync-'.uniqid().'.poachy.test', 'tenant_id' => $this->tenantId,
    ...
    private function createInboundSync(array $overrides = []): SyncQueueInbound
    {
        ...
        return SyncQueueInbound::on('central')->create(array_merge([
            'tenant_id' => $this->tenantId,
            'syncable_type' => 'Variant',
            'tenant_syncable_id' => 10,
            'action' => 'create',
            'payload' => $payload,
            'metadata' => ['sync_queue_id_from_tenant' => 702],
            'priority' => 3,
            'received_at' => now(),
            'scheduled_at' => now(),
    ...
    public function test_inbound_job_creates_variant_and_acks_completed(): void
    {
        Http::fake(['*/variant-ack' => Http::response(['success' => true], 200)]);
        $sync = $this->createInboundSync();

        (new ProcessInboundVariantSync($sync->id))->handle(app(MarketplaceVariantSyncService::class));
        ...
        Http::assertSent(function ($request) use ($product) {
            return str_contains($request->url(), 'variant-ack')
                && $request['outbound_sync_queue_id'] === 702
    ...
    public function test_inbound_job_acks_failed_after_max_retries(): void
    {
        ...
        Http::assertSent(function ($request) {
            return str_contains($request->url(), 'variant-ack') && $request['status'] === 'failed';
        });
    }
